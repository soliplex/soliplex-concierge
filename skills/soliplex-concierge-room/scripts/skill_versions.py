...
* ``list``  -- which versions have been published? Both the rolling builds
  (``room-skill-YYYY.MM.DD-<sha>``) and the snapshots attached to software
...
_USER_AGENT = "soliplex-concierge-room-skill"
...
# Rolling build tags look like ``room-skill-2026.05.29-abc1234``.
_ROLLING_RE = re.compile(r"^room-skill-\d{4}\.\d{2}\.\d{2}-[0-9a-f]+$")
...
class PointerUnavailable(SystemExit):
    """The ``room-skill-latest`` pointer manifest could not be read."""
...
def _resolve_target(
    ...
    no explicit ``asset_url`` was supplied, the ``room-skill-latest``
...
def cmd_upgrade(args: argparse.Namespace) -> int:
    ...
    print(f"Upgraded soliplex-concierge-room to {summary}.")
    return 0
