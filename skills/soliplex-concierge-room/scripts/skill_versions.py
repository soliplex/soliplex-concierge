...
# ///
"""List, diff, and upgrade ``soliplex-concierge-room`` skill versions.
...
* ``list``    -- which versions have been published? Rolling builds
  (``room-skill-YYYY.MM.DD-<sha>``) and release snapshots are shown
...
SPEC = versions.SkillSpec(
    ...
    rolling_re=re.compile(r"^room-skill-\d{4}\.\d{2}\.\d{2}-[0-9a-f]+$"),
    compare_scope="tree",
)
