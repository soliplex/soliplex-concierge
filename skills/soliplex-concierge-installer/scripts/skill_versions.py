...
# ///
"""List, diff, and upgrade ``soliplex-concierge-installer`` skill versions.
...
* ``list``    -- which versions have been published? Rolling builds
  (``installer-skill-YYYY.MM.DD-<sha>``) and release snapshots are shown
...
# is handled by the library. ``compare_scope="tree"`` because the concierge
# skill's payload is its whole tree (SKILL.md, scripts/, assets/).
SPEC = versions.SkillSpec(
    owner="soliplex",
    repo="soliplex-concierge",
    skill_name="soliplex-concierge-installer",
    asset_tarball="soliplex-concierge-installer-skill.tar.gz",
    pointer_tag="installer-skill-latest",
    rolling_re=re.compile(r"^installer-skill-\d{4}\.\d{2}\.\d{2}-[0-9a-f]+$"),
    compare_scope="tree",
)
