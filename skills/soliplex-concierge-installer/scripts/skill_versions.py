#!/usr/bin/env python
"""List, diff, and upgrade published versions of the
soliplex-concierge-installer skill.
...
* ``list``  -- which versions have been published? Both the rolling builds
  (``installer-skill-YYYY.MM.DD-<sha>``) and the snapshots attached to software
...
* ``diff``  -- how does the installed skill differ from a published version
  (default: ``latest``)? The whole skill tree (SKILL.md, scripts/, and any
  assets/) is compared.
...
_USER_AGENT = "soliplex-concierge-installer-skill"
...
# Rolling build tags look like ``installer-skill-2026.05.29-abc1234``.
_ROLLING_RE = re.compile(r"^installer-skill-\d{4}\.\d{2}\.\d{2}-[0-9a-f]+$")
...
class PointerUnavailable(SystemExit):
    """The ``installer-skill-latest`` pointer manifest could not be read."""
...
def _resolve_target(
    ...
    no explicit ``asset_url`` was supplied, the ``installer-skill-latest``
...
def cmd_upgrade(args: argparse.Namespace) -> int:
    ...
    print(f"Upgraded soliplex-concierge-installer to {summary}.")
    return 0
