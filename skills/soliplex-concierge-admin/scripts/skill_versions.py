...
* ``list``  -- which versions have been published? Both the rolling builds
  (``admin-skill-YYYY.MM.DD-<sha>``) and the snapshots attached to software
...
_USER_AGENT = "soliplex-concierge-admin-skill"
...
# Rolling build tags look like ``admin-skill-2026.05.29-abc1234``.
_ROLLING_RE = re.compile(r"^admin-skill-\d{4}\.\d{2}\.\d{2}-[0-9a-f]+$")
...
class PointerUnavailable(SystemExit):
    """The ``admin-skill-latest`` pointer manifest could not be read."""
...
def _resolve_target(
    ...
    no explicit ``asset_url`` was supplied, the ``admin-skill-latest``
...
def cmd_upgrade(args: argparse.Namespace) -> int:
    ...
    print(f"Upgraded soliplex-concierge-admin to {summary}.")
    return 0
